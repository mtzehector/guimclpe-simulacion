...
@Provider
public class ConsultarSolicitudExcelService extends ServiceDefinition<ResumenSimulacionRequest, ResumenSimulacionRequest> {
    ...
    public Message<ResumenSimulacionRequest> execute(Message<ResumenSimulacionRequest> request) throws BusinessException {
        log.log(Level.INFO, ">>>>ConsultarSolicitudService  request.getPayload().getSolicitud().getId()={0}", request.getPayload().getSolicitud().getId());
        ...
        if (respuesta.getStatus() == 200) {
            ...
            if (solicitudEntity.getCvePromotor() != null) {
                Response responsePromotorClient = promotorClient.obtenerPorClaveNoImage(solicitudEntity.getCvePromotor());
